...
public class FictionalImageFormat extends AbstractFormat {
	 ...
	 * <p>
	 * The header contains the metadata of the file. You need to know its size so
	 * that you can start reading image data from the right position.
	 * </p>
	 */
	 ...
	 * <p>
	 * Not necessary to {@link Override}, but a good practice since otherwise the
	 * name will be empty
	 * </p>
	 */
	@Override
	 ...
	 * example, things like acquisition date, instrument, excitation levels, etc.
	 * In the implementation of populateImageMetadata, the format-specific
	 * metadata is converted to a generalized ImageMetadata which can be consumed
	 * by other components (e.g. readers/writers). As the conversion to
	 ...
	 * <p>
	 * Each format-specific field in your Metadata class should be private, with a
	 * public get(), and set() methods if necessary. These fields should be
	 * annotated with {@link io.scif.Field} notations, and a label indicating the
	 * field's original name (as it might not be properly represented by Java
	...
	public static class Metadata extends AbstractMetadata {
		...
		public void populateImageMetadata() {
			...
			// Add CalibratedAxes to the image. They describe the dimensions of
			// the
			// image. Each dimension can have a type (spatial i.e. X,Y,Z,
			// Channel,
			// Time...), unit and a scale. Adding scales and units is similar to
			// calling pixelWidth, setUnit etc. in the IJ1 Calibration class.
			// NB The ImageMetadata needs to have at least the X and Y axes
			// present
	 ...
	 * purpose: to take the raw image information and generate a {@code Metadata}
	 * instance, populating all format-specific fields.
	 */
	public static class Parser extends AbstractParser<Metadata> {
			...
			SCIFIOConfig config) throws IOException, FormatException
		{
			...
			// the
			// range min-max to screen values in the range 0-255. Depending on
			// your
			// format, the image may show really dark without this setting.
			// Similar
			...
			// If this flag is set, it signifies a desire to save the raw
			// original
	 ...
	 * extension checking to determine compatibility, you do not need to implement
	 * any methods in this class - it's already handled for you in the
	...
	public static class Checker extends AbstractChecker {
			...
			throws IOException
		{
			...
			// 0xE, we check that the file is of this format by reading its
			// first
			// six bytes and comparing them to this sequence
			final byte[] firstBytes = new byte[FIF_ID.length];
			// File might be shorter than 6 bytes, so check that at least six
			// bytes
			// got read
	 ...
	 * to byte[], so a {@link ByteArrayReader} is typically faster and the default
	 * choice here. But select the class that makes the most sense for your
	 * format.
	 */
	...
	/**
	 * Writers are not implemented for proprietary formats, as doing so typically
	 * violates licensing. However, if your format is open source you are welcome
	...
	/**
	 * The purpose of a {@link Translator} is similar to that of a {@link Parser}:
	 * to populate the format-specific metadata of a {@link Metadata} object.
	 * However, while a {@link Parser} reads from an image source to perform this
	 * operation, a {@link Translator} reads from a {@link Metadata} object of
	 * another format.
	 * <p>
	 ...
	 * then be called whenever SCIFIO writes out your format, and it will be able
	 * to handle any input format type. Essentially this is translating
	 * {@link ImageMetadata} to your format-specific metadata.</li>
	 * <li>If you are adding support for a new {@link Metadata} schema to SCIFIO,
	 * you will probably want to create Translators to and from your new
		...
		AbstractTranslator<io.scif.Metadata, Metadata>
	{
			...
			AxisType type)
		{
			...
			// Return length as is if the calibration isn't in the same unit as
			// in
			// the format. Could use UnitService to try to convert the
			// calibration
